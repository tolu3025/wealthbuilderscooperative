-- Fix the distribute_mlm_earnings function to exclude payer from receiving their own payment distribution
...
BEGIN
  ...
  -- CRITICAL FIX: Exclude the payer (v_member_id) from receiving their own distribution
  FOR v_participant IN
    ...
    WHERE mt.member_id != company_id
      AND mt.member_id != v_member_id  -- EXCLUDE THE PAYER from receiving their own payment distribution
...
-- Step 1: Delete all existing MLM distributions (both member and company)
DELETE FROM public.mlm_distributions;

-- Step 2: Reset mlm_distributed flag for all approved PSF payments
UPDATE public.project_support_contributions
SET mlm_distributed = false
WHERE payment_status = 'approved';

-- Step 3: Re-run distribution for each approved PSF payment in chronological order
DO $$
DECLARE
