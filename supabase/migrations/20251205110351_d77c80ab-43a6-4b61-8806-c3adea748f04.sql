...
-- Update the distribute_mlm_earnings function to include the payer in distribution
...
BEGIN
  ...
  -- FIRST: Give the payer their ₦30 immediately (they're included in their own pool)
  INSERT INTO public.mlm_distributions (
    project_support_payment_id,
    member_id,
    amount,
  ...
  v_distributed_count := 1; -- Payer counts as 1 slot
  
  -- Distribute to OTHER eligible members who have paid BEFORE this payment in the same month
  FOR v_participant IN
    ...
    WHERE mt.member_id != company_id
      AND mt.member_id != v_member_id  -- Exclude the payer (already received)
    ...
    LIMIT 8  -- 8 more slots (payer already took 1 of the 9 member slots)
  LOOP
    INSERT INTO public.mlm_distributions (
      project_support_payment_id,
      member_id,
